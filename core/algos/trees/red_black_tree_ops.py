    ...
    for i, raw in enumerate(text.splitlines(), start=1):
        ...
        try:
            ...
            raise ValueError(
                ...
                "Usa insert/delete/contains/trace/min/max/inorder/bfs/clear."
            ) from err
...
Handler = Callable[[RedBlackTree[Any], Operation], tuple[str, list[Any]]]
# handler devuelve (mensaje, highlight_values)


def _h_insert(t: RedBlackTree[Any], op: Operation) -> tuple[str, list[Any]]:
    ok = t.insert(op.value)
    return (f"insert {op.value} → {'OK' if ok else 'YA EXISTE'}", [])


def _h_delete(t: RedBlackTree[Any], op: Operation) -> tuple[str, list[Any]]:
    ok = t.delete(op.value)
    return (f"delete {op.value} → {'OK' if ok else 'NO ENCONTRADO'}", [])


def _h_contains(t: RedBlackTree[Any], op: Operation) -> tuple[str, list[Any]]:
    ok = t.contains(op.value)
    trace = t.search_trace(op.value)
    return (f"contains {op.value} → {ok}", trace)


def _h_trace(t: RedBlackTree[Any], op: Operation) -> tuple[str, list[Any]]:
    trace = t.search_trace(op.value)
    found = bool(trace) and trace[-1] == op.value
    return (f"trace {op.value} → {'FOUND' if found else 'NOT FOUND'}", trace)


def _h_min(t: RedBlackTree[Any], _op: Operation) -> tuple[str, list[Any]]:
    try:
        v = t.min_value()
        return (f"min → {v}", [v])
...
def _h_max(t: RedBlackTree[Any], _op: Operation) -> tuple[str, list[Any]]:
    try:
        v = t.max_value()
        return (f"max → {v}", [v])
    except ValueError as e:
...
def _h_inorder(t: RedBlackTree[Any], _op: Operation) -> tuple[str, list[Any]]:
    return (f"inorder → {t.inorder()}", [])


def _h_bfs(t: RedBlackTree[Any], _op: Operation) -> tuple[str, list[Any]]:
    return (f"bfs → {t.bfs()}", [])


def _h_clear(t: RedBlackTree[Any], _op: Operation) -> tuple[str, list[Any]]:
    t.clear()
    return ("clear", [])
...
def build_steps(ops: list[Operation], *, dot_builder: callable) -> list[Step]:
    t: RedBlackTree[Any] = RedBlackTree()
    ...
    for op in ops:
        ...
        except Exception as e:  # demo/visualizador
