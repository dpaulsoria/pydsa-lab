...
def parse_operations(text: str) -> list[Operation]:
    ...
    for i, raw in enumerate(text.splitlines(), start=1):
        ...
        try:
            ...
            else:  # POP_FRONT / POP_BACK / REVERSE
...
Handler = Callable[[DoublyLinkedList[Any], Operation], tuple[str, int | None]]


def _h_push_front(dll: DoublyLinkedList[Any], op: Operation) -> tuple[str, int | None]:
    dll.push_front(op.value)
    return (f"push_front {op.value}", 0)


def _h_push_back(dll: DoublyLinkedList[Any], op: Operation) -> tuple[str, int | None]:
    dll.push_back(op.value)
    return (f"push_back {op.value}", len(dll) - 1)


def _h_pop_front(dll: DoublyLinkedList[Any], op: Operation) -> tuple[str, int | None]:
    removed = dll.pop_front()
    return (f"pop_front → {removed}", 0 if len(dll) else None)


def _h_pop_back(dll: DoublyLinkedList[Any], op: Operation) -> tuple[str, int | None]:
    removed = dll.pop_back()
    return (f"pop_back → {removed}", len(dll) - 1 if len(dll) else None)


def _h_delete(dll: DoublyLinkedList[Any], op: Operation) -> tuple[str, int | None]:
    ok = dll.delete(op.value)
    return (f"delete {op.value} → {'OK' if ok else 'NO ENCONTRADO'}", None)


def _h_delete_all(dll: DoublyLinkedList[Any], op: Operation) -> tuple[str, int | None]:
    count = dll.delete_all(op.value)
    return (f"delete_all {op.value} → {count}", None)


def _h_delete_at(dll: DoublyLinkedList[Any], op: Operation) -> tuple[str, int | None]:
    idx = int(op.value)
    removed = dll.delete_at(idx)
    return (f"delete_at {idx} → {removed}", _clamp_hi(idx, len(dll)))


def _h_find(dll: DoublyLinkedList[Any], op: Operation) -> tuple[str, int | None]:
    idx = dll.find_index(op.value)
    msg = f"find {op.value} → {idx if idx is not None else 'NO ENCONTRADO'}"
    return (msg, _clamp_hi(idx, len(dll)))


def _h_reverse(dll: DoublyLinkedList[Any], op: Operation) -> tuple[str, int | None]:
    dll.reverse()
    return ("reverse", None)
...
def build_steps(ops: list[Operation], dot_builder: callable) -> list[Step]:
    ...
    for op in ops:
        try:
            msg, hi = HANDLERS[op.kind](dll, op)
